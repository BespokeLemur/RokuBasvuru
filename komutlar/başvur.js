...
exports.run = async (client, message, args) => { 
  ...
  const sorular = [
    ...
    '**Üyelere hangi konuda destek vermeyi planlıyorunuz?**',
    '**Kendinizi Bize Tanıtın?** <cevabınız>',
    '**Neden Roku?** <cevabınız>'
  ]
  let sayac = 0
  ...
  collector.on('end', collected => {
    ...
    .addField('Başvuran Hakkında',[
      ...
      `**Kendinizi Bize Tanıtın: **\t\t${collected.map(m => m.content).slice(5,6)}`,
      `**Neden Roku: **\t\t${collected.map(m => m.content).slice(6,7)}`
    ])
    .setTimestamp()
    .setFooter('Developed by BespokeLemur', message.guild.iconURL());
